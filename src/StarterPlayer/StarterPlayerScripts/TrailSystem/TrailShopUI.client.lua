...
local function purchaseTrail(trailId)
	...
	if purchaseInProgress[trailId] then
		print("Purchase already in progress for " .. trailData.name)
		return
	end

	-- Check if trail is already owned
	if ownedTrails[trailId] then
		print("Trail " .. trailData.name .. " is already owned")
		return
	end
	...
	if not frame then
		warn("Frame not found for trail: " .. trailId)
		return
	end
...
local function equipTrail(trailId)
	...
	if currentEquippedTrail == trailId then
		print("Trail " .. trailData.name .. " is already equipped")
		return
	end

	-- Check if equipment is already in progress for this trail
	if equipInProgress[trailId] then
		print("Trail equipment already in progress for " .. trailData.name)
		return
	end
