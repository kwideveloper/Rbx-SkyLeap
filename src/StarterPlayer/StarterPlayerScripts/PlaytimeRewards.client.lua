...
local claimingButtons = {} -- To prevent button spam
...
local function applyState(container, state, remaining, showButtonTime)
	...
	if activeGrad and activeGrad:IsA("UIGradient") then
		-- All active/claimable items have ActiveGradient, only next shows button time
		local shouldEnable = (state == "Active" or state == "Claimable")
		activeGrad.Enabled = shouldEnable

		-- Start infinite rotation for both active and claimable items
		if shouldEnable then
			if not gradientTweens[activeGrad] then
				-- Reset to 0 first for smooth start
				activeGrad.Rotation = 0
				local tweenInfo = TweenInfo.new(
					1.5, -- Duration (1.5 seconds for full rotation, faster)
					Enum.EasingStyle.Linear,
					Enum.EasingDirection.InOut,
					-1, -- Infinite repetitions
					false -- No reverse - keeps going forward for smooth loop
				)
				local goal = { Rotation = 360 }
				local tween = TweenService:Create(activeGrad, tweenInfo, goal)
				gradientTweens[activeGrad] = tween

				-- Handle smooth loop by resetting rotation when tween completes
				tween.Completed:Connect(function()
					if gradientTweens[activeGrad] == tween then
						activeGrad.Rotation = 0
					end
				end)
			...
			-- Stop rotation if not active/claimable
		...
		if state == "Claimable" then
			if btn then
				btn.MouseButton1Click:Connect(function()
					...
					-- Prevent button spam
					if claimingButtons[btn] then
						return
					end
					claimingButtons[btn] = true

					-- Trigger animation immediately for instant feedback
					print("DEBUG: Triggering animation immediately for", r.amount, r.type)
					RewardAnimations.spawnRewardBurst(r.amount, r.type, nil, btn)

					-- Immediate UI update for better responsiveness
					applyState(clone, "Completed", 0, r.next == true)
